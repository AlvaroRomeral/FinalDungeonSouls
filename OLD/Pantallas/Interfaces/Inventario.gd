...
func _ready():
	hide()
	Jugador.connect("inventarioActualizado", Callable(self, "actualizarSlots"))
	Jugador.connect("datosActualizados", Callable(self, "actualizarDatos"))
	generarSlots()
	actualizarSlots()
...
func mostrarInfo(id_item):
	if id_item != null:
		...
		match Datos.getItemTipo(id_item):
			"consumible":
				if datos["vida"] != 0:
					txtestats.text = "[center]Vida: [color=red]" + str(datos["vida"])
				else:
					txtestats.text = "[center]Mana: [color=blue]" + str(datos["mana"])
			"arma":
				txtestats.text = "[center]Ataque: " + str(datos["ataque"])
			"torso":
				txtestats.text = "[center]Defensa: " + str(datos["defensa"])
			"cabeza":
				txtestats.text = "[center]Defensa: " + str(datos["defensa"])
			"piernas":
				txtestats.text = "[center]Defensa: " + str(datos["defensa"])
			...
			"amuleto":
				match datos["efecto"]:
					"vida":
						txtestats.text = "[center]Vida extra: [color=red]" + str(datos["porcen"]*100) + "%"
	else:
		txtnombre.text = ""
		txtdescri.text = ""
		txtestats.text = ""
