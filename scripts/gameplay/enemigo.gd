...
func _ready():
	control_estado.just_muerto.connect(morir)
...
func _physics_process(_delta):
	...
	if jugador:
		...
		if nav_agent.is_target_reachable() and global_position.distance_to(jugador.global_position) > 20 and !jugador.control_estado.is_muerto():
		...
		if global_position.distance_to(jugador.global_position) < 30 and !jugador.control_estado.is_muerto():
